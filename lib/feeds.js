...
export async function getUpdatesFromDb(categoryFilter = null, take = 80) {
  try {
    const where = categoryFilter && categoryFilter !== 'general' ? { category: categoryFilter } : {}
    const docs = await prisma.update.findMany({
      where,
    ...
    return docs.map(doc => ({
      title: doc.title,
      description: doc.description || doc.summaryEn,
      feedCategory: doc.category,
