...
const categoryMap = {
  'Azure Blog': { badge: 'bg-ms-blue/15 text-ms-blue', label: 'Azure', cat: 'azure', gradient: 'linear-gradient(135deg, #0078D4 0%, #00BCF2 100%)' },
  'Windows Blog': { badge: 'bg-ms-green/15 text-ms-green', label: 'Windows', cat: 'windows', gradient: 'linear-gradient(135deg, #00A4EF 0%, #7FBA00 100%)' },
  'Microsoft Security': { badge: 'bg-[#00BCF2]/15 text-[#00BCF2]', label: 'Security', cat: 'security', gradient: 'linear-gradient(135deg, #F25022 0%, #FFB900 100%)' },
  'Microsoft 365 Dev': { badge: 'bg-ms-orange/15 text-ms-orange', label: 'Office 365', cat: 'office365', gradient: 'linear-gradient(135deg, #FFB900 0%, #F25022 100%)' },
  'Power Platform': { badge: 'bg-ms-yellow/15 text-ms-yellow', label: 'Power Platform', cat: 'power-platform', gradient: 'linear-gradient(135deg, #742774 0%, #C084FC 100%)' },
  'Microsoft Dev Blogs': { badge: 'bg-ms-accent/15 text-ms-accent', label: 'Developer', cat: 'copilot', gradient: 'linear-gradient(135deg, #A855F7 0%, #22D3EE 100%)' },
  'Copilot & AI': { badge: 'bg-ms-accent/15 text-ms-accent', label: 'Copilot / AI', cat: 'copilot', gradient: 'linear-gradient(135deg, #22D3EE 0%, #A855F7 100%)' },
  'Microsoft Fabric': { badge: 'bg-ms-accent/15 text-ms-accent', label: 'Fabric', cat: 'fabric', gradient: 'linear-gradient(135deg, #C084FC 0%, #00BCF2 100%)' },
}
...
export default function HeroSection({ articles = [] }) {
  ...
  const heroMeta = categoryMap[hero.source] || { badge: 'bg-ms-accent/15 text-ms-accent', label: hero.source, gradient: 'linear-gradient(135deg, #A855F7, #22D3EE)' }
  const heroSlug = hero.slug || makeSlug(hero.title)
  const heroCat = hero.feedCategory || heroMeta.cat || 'general'
  const heroIcon = categoryIcons[heroCat] || '📰'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="flex flex-col gap-4">
          {sideItems.map((article, i) => {
            const meta = categoryMap[article.source] || { badge: 'bg-ms-accent/15 text-ms-accent', label: article.source, gradient: 'linear-gradient(135deg, #A855F7, #22D3EE)' }
            const artSlug = article.slug || makeSlug(article.title)
            const artCat = article.feedCategory || meta.cat || 'general'
            const artIcon = categoryIcons[artCat] || '📰'
