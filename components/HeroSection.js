  ...
  return (
    <section className="max-w-7xl mx-auto px-4 pt-8 pb-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <a
          href={`/live/${heroSlug}`}
          className="group lg:col-span-2 bg-ms-card rounded-2xl border border-[var(--border)] overflow-hidden hover:border-ms-blue/40 transition-all duration-300 block"
        >
          {/* Hero image */}
          {hero.image && (
            <div className="w-full aspect-[16/7] overflow-hidden bg-ms-dark">
              <img src={hero.image} alt={hero.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="eager" />
            </div>
          )}
          ...
          {sideItems.map((article, i) => {
            ...
            return (
              <a
                key={i}
                href={`/live/${artSlug}`}
                className="group bg-ms-card rounded-xl border border-[var(--border)] overflow-hidden hover:border-ms-blue/40 transition-all duration-200 hover:-translate-y-0.5 block"
              >
                {article.image && (
                  <div className="w-full aspect-[16/7] overflow-hidden bg-ms-dark">
